        ...
        function obj=draw(obj)
            ...
            for i=obj.ridgeNum:-1:1
                ...
                switch obj.ColorMode
                    ...
                    case 'X'
                        tTi=[Xi(1),Xi,Xi(end)]-min(Xi);
                        tTi=tTi./max(tTi);
                        tT=linspace(0,1,size(obj.ColorList,1));
                        tR=interp1(tT,obj.ColorList(:,1),tTi);
                        tG=interp1(tT,obj.ColorList(:,2),tTi);
                        tB=interp1(tT,obj.ColorList(:,3),tTi);
                        tC=cat(3,[tR;tR],[tG;tG],[tB;tB]);
                        obj.ridgePatchHdl(i)=surf([Xi(1),Xi,Xi(end);Xi(1),Xi,Xi(end)],[0,F.*0,0;0,F,0]+obj.Sep.*(i).*ones(1,length(F)+2),[tTi;tTi].*0,...
                            'CData',tC,'EdgeColor','none','FaceAlpha',.9,'FaceColor','interp');
                        obj.ridgeLineHdl(i)=plot([Xi(1),Xi,Xi(end)],[0,F,0]+obj.Sep.*(i).*ones(1,length(F)+2),...
                    ...
                    case 'Qt'
                        ...
                        tC=cat(3,[tR;tR],[tG;tG],[tB;tB]);
                        obj.ridgePatchHdl(i)=surf([tTi;tTi],[0,F.*0,0;0,F,0]+obj.Sep.*(i).*ones(1,length(F)+2),[tTi;tTi].*0,...
                            'CData',tC,'EdgeColor','none','FaceAlpha',.9,'FaceColor','interp');
                        obj.ridgeLineHdl(i)=plot([Xi(1),Xi,Xi(end)],[0,F,0]+obj.Sep.*(i).*ones(1,length(F)+2),...
                            'Color',[0,0,0,.9],'LineWidth',.8);
                        colormap(obj.ColorList);
