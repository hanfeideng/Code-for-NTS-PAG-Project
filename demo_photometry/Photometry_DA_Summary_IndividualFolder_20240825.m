...
for ii = 1:length(trigger_evt)-1
    evt_start_time_match_ind1(ii) = evt_start_time_match_ind(ii+1,1) - sham_pre * freq;
end
...
%         time1 = time(time>-2);
    ...
    for jj = 1:2
        ...
        trough_ind = find(ROC == min(ROC));
        trough = ROC(trough_ind);
...
writematrix(TROUGH_Sum,'PEAK_Sum.csv');
